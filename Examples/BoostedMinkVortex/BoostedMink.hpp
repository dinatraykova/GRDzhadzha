...
#ifndef BOOSTEDMINK_HPP_
#define BOOSTEDMINK_HPP_
...
class BoostedMink
{
  public:
    ...
    BoostedMink(params_t a_params, double a_dx) : m_params(a_params), m_dx(a_dx)
    {
                                   ...
                                   const Coordinates<data_t> &coords) const
    {
        ...
        data_t sign_lapse = 1.;  //(r - 0.5 * M) / abs(r - 0.5 * M);
        vars.lapse = sign_lapse; //* sqrtA / boost * sqrt(B / (B - A * v2));
        ...
        // FOR1(i) { vars.shift[i] = delta(i, 0) * A * v; } /// boost2 / (B - A
        // * v2); }
        ...
        vars.K = 0; // compute_trace(gamma_UU, vars.K_tensor);
    }
};

#endif /* BOOSTEDMINK_HPP_ */
