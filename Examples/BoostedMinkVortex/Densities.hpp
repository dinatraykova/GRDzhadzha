...
#ifndef DENSITIES_HPP_
#define DENSITIES_HPP_
...
template <class matter_t, class background_t> class Densities
{
  ...
  public:
    Densities(matter_t a_matter, background_t a_background, double a_dx,
    ...
    template <class data_t> void compute(Cell<data_t> current_cell) const
    {
        ...
        data_t rhoParticle =
            -vars.phi_Re * vars.Pi_Im + vars.phi_Im * vars.Pi_Re;
        rhoParticle *= sqrt(det_gamma);

        // Store the values on the grid
        current_cell.store_vars(emtensor.rho * sqrt(det_gamma), c_rhoDensity);
        current_cell.store_vars(rhoParticle, c_rhoParticle);
        current_cell.store_vars(rhoEnergy, c_rhoEnergy);
    }
};

#endif /* DENSITIES_HPP_ */
