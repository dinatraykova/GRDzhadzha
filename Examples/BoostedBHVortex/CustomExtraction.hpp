...
class CustomExtraction
{
  ...
  public:
    //! The constructor
    CustomExtraction(int a_comp, int a_num_points, double a_R,
        ...
        : m_comp(a_comp), m_num_points(a_num_points), m_origin(a_origin),
          m_R(a_R), m_dt(a_dt), m_time(a_time), m_restart_time(a_restart_time)
    {
    }
                       ...
                       std::string a_file_prefix) const
    {
        ...
        std::vector<double> theta(m_num_points);

        // Work out the coordinates
        // go around a circle of radius R in the x-y plane
        for (int idx = 0; idx < m_num_points; ++idx)
        {
            theta[idx] = double(idx) / double(m_num_points) * 2. * M_PI;
            interp_x[idx] = m_origin[0] + m_R * cos(theta[idx]);
            interp_y[idx] = m_origin[1] + m_R * sin(theta[idx]);
        ...
        if (first_step)
        {
            for (int i = 0; i < m_num_points; ++i)
            {
                header_line[i] =
                    ...
                    to_string_with_precision(interp_y[i], 2) + "," +
                    to_string_with_precision(interp_z[i], 2) + "), ";
            }
